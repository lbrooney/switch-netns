...
int main(int argc, char** argv) {
	...
	if (setns(params.namespace_fd, CLONE_NEWNET) != 0) {
		...
		const char* setns_error_description = NULL;
		switch (error) {
			case EBADF:  setns_error_description = "\tfd is not a valid file descriptor."; break;
			case EINVAL: 
				setns_error_description = "\tfd refers to a namespace whose type does not match that specified in nstype; or \n"
					"\tThere is problem with reassociating the thread with the specified namespace; or \n"
					"\tThe caller tried to join an ancestor (parent, grandparent, and so on) PID namespace; or \n"
					"\tThe caller attempted to join the user namespace in which it is already a member; or \n"
					"\tThe caller shares filesystem (CLONE_FS) state (in particular, the root directory) with other processes and tried to join a new user namespace; or \n"
					"\tThe caller is multithreaded and tried to join a new user namespace; or \n"
					"\tfd is a PID file descriptor and nstype is invalid (e.g., it is 0)."; 
			break;
			case ENOMEM: setns_error_description = "\tCannot allocate sufficient memory to change the specified namespace."; break;
			case EPERM:  setns_error_description = "\tThe calling thread did not have the required capability for this operation."; break;
			case ESRCH:  setns_error_description = "\tfd is a PID file descriptor but the process it refers to no longer exists (i.e., it has terminated and been waited on)."; break;
		}
...
static LaunchParams parse_launch_params(const char* program_name, int argc, char** argv) {
	...
	if (args_info.by_file_given) {
		...
		fprintf(stderr, "No network namespace resolution way (`--by-name`, `--by-file`, or `--by-pid`) provided.");
		show_usage(program_name);
		exit(7);
	}

	if (filepath == NULL)
		panic("Internal error: Failed to establish filepath of network namespace.")
...
static void check_capabilities(cap_value_t* required_caps, size_t count,
	...
	for (size_t i = 0; i < count; i++) {
		int capability_code = required_caps[i];
